...
- [ ] **1.2** Configure lint-staged in `package.json`
...
- [ ] **2.1** Create `scripts/check-file-lines.sh`
...
- [ ] **4.1** Run full verification against acceptance criteria
...
- [ ] `cargo clippy` passes on full codebase
- [ ] `cargo fmt -- --check` passes on full codebase
- [ ] No source file in `src/` or `src-tauri/src/` exceeds 500 lines (pre-existing violations must be fixed or exempted)
