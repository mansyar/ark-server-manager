...
use crate::services::steamcmd_installer::{
    install_steamcmd, sanitize_install_path, InstallerConfig,
};
...
pub async fn install_server(
    ...
    let steamcmd_path =
        ensure_steamcmd(profile.steamcmd_path.as_ref()).map_err(|e| e.to_string())?;
    ...
    let config = RunnerConfig::new(steamcmd_path.clone()).with_install_dir(install_dir.clone());
    ...
    debug!(
        "Running steamcmd install script for profile: {}",
        profile_name
    );
    ...
    match result {
        Ok(output) => {
            ...
            if let Ok(build_id) = get_current_build_id(&steamcmd_path, &install_dir) {
                let mut profile =
                    load_profile_internal(&profile_name).map_err(|e| e.to_string())?;
...
#[tauri::command]
pub async fn update_server(profile_name: String, app: AppHandle) -> Result<SteamCmdResult, String> {
    ...
    if is_running {
        return Err(
            "Cannot update: server is currently running. Stop the server first.".to_string(),
        );
    }
    ...
    let result =
        crate::services::update_manager::update_server(update_config).map_err(|e| e.to_string())?;
...
pub async fn verify_server(
    ...
    let result = run_steamcmd_script(&config, &[&script], None).map_err(|e| e.to_string())?;
...
#[tauri::command]
pub async fn get_steamcmd_status(profile_name: String) -> Result<SteamCmdStatus, String> {
    ...
    let (update_available, current_build, available_build) =
        if let (Some(sc_path), Some(dir)) = (steamcmd_path.as_ref(), install_dir.as_ref()) {
            let sc_path_buf = PathBuf::from(sc_path);
            match check_build_ids(&sc_path_buf, dir) {
                Ok(info) => (
                    info.update_available,
                    info.current_build,
                    info.available_build,
                ),
...
fn load_profile_internal(name: &str) -> Result<Profile, SteamCmdError> {
    ...
    let contents =
        std::fs::read_to_string(&path).map_err(|e| SteamCmdError::IoError(e.to_string()))?;
...
fn save_profile_internal(profile: Profile) -> Result<(), SteamCmdError> {
    ...
    if !dir.exists() {
        std::fs::create_dir_all(&dir).map_err(|e| SteamCmdError::IoError(e.to_string()))?;
    }
    let path = dir.join(format!("{}.json", profile.name));
    let json = serde_json::to_string_pretty(&profile)
        .map_err(|e| SteamCmdError::IoError(e.to_string()))?;
    std::fs::write(&path, json).map_err(|e| SteamCmdError::IoError(e.to_string()))?;
    Ok(())
}
...
fn resolve_install_dir(profile: &Profile) -> Result<PathBuf, SteamCmdError> {
    ...
    if let Some(ref server_path) = profile.server_install_path {
        ...
        let root = if server_path
            .file_name()
            .map(|n| n == "ShooterGameServer.exe")
            .unwrap_or(false)
        {
            server_path
                .parent()
